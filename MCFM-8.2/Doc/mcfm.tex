...
\usepackage[style=numeric-comp,backend=biber,eprint=true,url=true]{biblatex}
...
\label{subsec:hjetma}
This process represents the production of a Higgs boson in association with a single jet based on refs.~\cite{Neumann:2016dny,Neumann:2018}. Decay modes are currently unsupported/untested. The top-quark mass is taken into account exactly for the born and real-emission parts, as well as for the singular part of the virtual corrections.
...
\begin{itemize}
    ...
    \item In a high energy expansion by setting mtex=100 in the input file. This is recommended for transverse momenta beyond \SI{500}{\GeV}.
    \item In a rescaling approach where the effective field theory ($m_t=\infty$) result is rescaled pointwise by the ratio of the one-loop amplitude computed with full $m_t$ dependence to the one-loop amplitude for $m_t=\infty$. This mode is the default mode and enabled with mtex=0 in the input file. It is the recommended approach for quantities that are sentitive to either region or intermediate energies.
\end{itemize}
...
\label{changelog8.2}
\begin{itemize}
    \item Implemented H+jet high energy amplitudes. See description for process 200.
\end{itemize}
